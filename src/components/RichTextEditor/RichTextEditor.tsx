...
import {
  ...
  List, ListOrdered, Undo, Redo, Palette, Highlighter
} from "lucide-react"
import { cn } from "@/lib/utils"
import { useRichTextEditor } from "@/hooks/useRichTexEditor"
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1 p-1 border-b bg-muted/50">
        ...
        {/* Colores */}
        <ColorPickerButton command="foreColor" icon={<Palette className="h-4 w-4" />} label="Color de texto" colors={TEXT_COLORS} onFormat={handleFormat} />
        <ColorPickerButton command="backColor" icon={<Highlighter className="h-4 w-4" />} label="Color de fondo" colors={TEXT_COLORS} onFormat={handleFormat} />
        ...
        {/* Botón IA */}
        <AIGenerateButton loading={loading} onClick={handleGenerateContent} />
      </div>
      ...
      {/* Barra de estado con contador de caracteres */}
